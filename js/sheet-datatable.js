function makeDataTable(tableid, dropdown = false) {
...
let noVis = [];
$(table).find('th.noVis').each(function () { noVis.push($(this).index()); });
let orderColumns;
        ...
        if (!tr.hasClass('loaded')) {
            if (isDatabase) {
                transform(xml, xslTable, { id: tr.attr("id") }).then(function (linkedsheet) {
                    ...
                    makeDataTable(tr.next('tr').find('table.linkedsheet').attr("id"), true);
                }, function (error) {
